...
const COLS = [
  ...
  "Final Row Flag", // X
  "Insurance", // Y
  "Auto Where to Bill", // Z  <-- was AA, "Where to Bill" (AA) removed
  "Billed?", // AA
  "DNS Reason", // AB
  "Note Link", // AC
  "Note", // AD
...
const META_ROW_2 = (function () {
  ...
  a[23] = "BILLING INPUTS";
  a[26] = "BILLING STATUS & SUBMISSION"; // shifted left by 1
  return a;
})();
...
function recomputeWhereToBill_Blanks_ThisBiller() {
  ...
  const autoRange = sheet.getRange(startRow, autoCol, numRows, 1);
  const autoVals = autoRange.getValues();
  for (let i = 0; i < numRows; i++) {
    const cur = String(autoVals[i][0] || "").trim();
    if (!cur) {
      autoVals[i][0] = routeWhereToBill_(String(insVals[i][0] || ""), String(apptVals[i][0] || ""), String(tcVals[i][0] || ""));
    }
  }
  autoRange.setValues(autoVals);
...
function runForConfig_(cfg, onlyNew) {
  ...
  if (rows.length) {
    ...
    const autoRange = sheet.getRange(startRow, autoCol, numRowsNew, 1);
    const autoVals = autoRange.getValues();
...
function ensureHeaderAndColumnO_(sheet) {
  ...
  // Break apart merges row by row across the full sheet width to avoid
  // "must select all cells in a merged range" errors when old merges
  // span a different column count than the current COLS.length.
  const sheetMaxCols = sheet.getMaxColumns();
  try { sheet.getRange(1, 1, 1, sheetMaxCols).breakApart(); } catch (e) {}
  try { sheet.getRange(2, 1, 1, sheetMaxCols).breakApart(); } catch (e) {}
  ...
  row1[26] = "BILLING STATUS & SUBMISSION"; // shifted left by 1 (no Where to Bill)
  sheet.getRange(1, 1, 1, COLS.length).setValues([row1]);
  sheet.getRange(2, 1, 1, COLS.length).setValues([COLS]);
  // Merge ranges updated: col 28 group is now col 27 (4 cols: AA–AD = indices 27–30)
  sheet.getRange(1, 1, 1, 5).merge();
  sheet.getRange(1, 10, 1, 2).merge();
  sheet.getRange(1, 24, 1, 3).merge(); // BILLING INPUTS: Y, Z, AA (Insurance, Auto WTB, Billed?) = 3 cols
  sheet.getRange(1, 27, 1, 4).merge(); // BILLING STATUS: AB–AD = DNS Reason, Note Link, Note = 4 cols
...
function ensureColumnGroups_(sheet) {
  ...
  // Groups adjusted: "Where to Bill" (col 26, index 25) removed
  // Group 1: cols F–N (indices 5–13, startIndex=5, endIndex=14) — unchanged
  // Group 2: cols O–W (indices 14–22, startIndex=14, endIndex=23) — unchanged
  // Group 3: cols Y–AD (indices 24–29, startIndex=24, endIndex=30) — shrunk by 1
  const requests = [
    { addDimensionGroup: { range: { sheetId, dimension: "COLUMNS", startIndex: 5, endIndex: 14 } } },
    { addDimensionGroup: { range: { sheetId, dimension: "COLUMNS", startIndex: 14, endIndex: 23 } } },
...
function ensureInsuranceAndBilledValidation_(sheet) {
  ...
  sheet.getRange(firstDataRow, autoCol, numRows, 1).clearDataValidations();
  ...
  const autoRule = SpreadsheetApp.newDataValidation().requireValueInList(["Headway","Lytec (Intake Only)","Lytec (Follow-ups Only)","Self Pay",""], true).setAllowInvalid(true).build();
  sheet.getRange(firstDataRow, autoCol, numRows, 1).setDataValidation(autoRule);
}
...
function detectCancelStatusInCurrentPanel_(text) {
  ...
  const searchArea = panel || topLines;
  let statusLine = "";
...
function findClinicianName_(text) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    for (var j = i - 1; j >= Math.max(0, i - 3); j--) {
      ...
      if (/^[×xX✕]\s/i.test(ln)) continue;
      if (/^[A-Z]{1,3}$/.test(ln)) continue;
